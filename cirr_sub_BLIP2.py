...
if __name__ == '__main__':
    ...
    testset = datasets.CIRR(path="/root/autodl-tmp/cirr_data/CIRR/",transform=preprocess)
    import sys
    model_dir = sys.argv[1]#"./output/cirr_blip2_pretrain_2024-06-11_09-29-51"
    import os
    
    file_ls = os.listdir(model_dir) 
    for i in file_ls:
        if ".pt" in i and f'CIRR_pred_ranks_recall{i[:-3]}.json' not in file_ls:
            model = torch.load(os.path.join(model_dir, i),weights_only=False)
            print(i[:-3] + " start")
